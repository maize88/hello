;(function () {
    ...
    function parse(el) {
      const result = { name: '', address: '', phone: '', age: '', birthday: '' }
      ...
      try {
        ...
        const ageList = el.querySelectorAll('h3')
        for (let i = 0; i < ageList.length; i++) {
          const e4 = ageList[i]
          const line = e4.innerText.trim()
  
          const match = /^age\s+(\d+)\s+(.*)/i.exec(line)
          if (match) {
            result.age = match[1]
            result.birthday = match[2].trim()
            break
          }
        }
      } catch (err) {
